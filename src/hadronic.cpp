 ...
 *
 *    Description:  Integration over the momentum fractions in order to get the final
 *                  hadronic results.
 *
 *        Version:  1.0
...
//----------------------------------------------------------------------------------------------//
double HadronicHiggs::ExtractDeltaHadronic(
        double pt,
        double tm,
        double t,
        double y1,
...
double deltaIntegrand(double t, double yc, void *p) {
    ...
    return par.hadrhiggs->ExtractDeltaHadronic(
            par.pt,
            par.tm,
            t,
            par.y1,
...
//----------------------------------------------------------------------------------------------//
double HadronicHiggs::ExtractDistrHadronic(
        double pt,
        double tm,
        double y1,
        double y2,
...
double distrIntegrand(double zz1, double zz2, double zz3, void *p) {
    HadronicStruct par =  *reinterpret_cast<HadronicStruct*>(p);
    return par.hadrhiggs->ExtractDistrHadronic(
            par.pt,
            par.tm,
            par.y1,
            par.y2,
...
std::vector<double> HadronicHiggs::higgsdpt(
        double pt,
        double y1,
        double y2
) {
    ...
    double xr = std::pow(1-xtau,2)-4*xtau*std::pow(pt/q,2);     // pt kinematic limit
    double ymax = log((tmpx+sqrt(std::pow(tmpx,2)-4))/2.);      // y kinematic limit
    ...
    // LO
    deltres = integration::IntegrateDeltaHadronic(
            method,
            deltaIntegrand,
            &finalparams,
            &delterr);
    ...
    // NLO
    if (ORD >= 1) {
        distres = integration::IntegrateDistrHadronic(
                method,
                distrIntegrand,
                &finalparams,
                &disterr);
