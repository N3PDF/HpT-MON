...
int integration::CubaIntegrator(
    ...
    int ndim, int ncomp, double prec, double **res,int *fail,
    double **error, double **prob, void * par, int verbos
) {
    ...
    switch (method) {
        case 0:
            Vegas(ndim,ncomp,func,par,nvec,prec,epsabs,verbose,seed,
                mineval, maxeval, nstart, nincrease,nbatch,gridno,
                statefile,NULL,&neval, fail,ris,err,proba);
        break;
        case 1:
            Suave(ndim,ncomp,func,par,nvec,prec,epsabs,verbose,seed,
                mineval, maxeval, nnew, 5,flatness,statefile,NULL,
        ...
        case 2:
            Divonne(ndim,ncomp,func,par,nvec,prec,epsabs,verbose,seed,
                    ...
                    maxchisq, mindeviation, ngiven, ldxgiven, NULL,
                    nextra, NULL,statefile,NULL, &nregions, &neval,
                    fail, ris, err, proba);
        break;
        case 3:
            Cuhre(ndim,ncomp,func,par,nvec,prec,epsabs,verbose | last,
                    mineval, maxeval, key, statefile, NULL,&nregions,
                    &neval, fail, ris, err, proba);
        break;
    }
    *error = err;
    *prob = proba;
...
//----------------------------------------------------------------------------------------------//
int DeltaPartonicIntegrand(int* ndim, double* x, int* ncomp, double* y, void* p) {
...
double integration::IntegrateDeltaPartonic(
        int method,
...
int DistrPartonicIntegrand(int* ndim, double* x, int* ncomp, double* y, void* p) {
...
double integration::IntegrateDistrPartonic(
        int method,
...
double integration::IntegrateDeltaHadronic(
        int method,
        double (Func)(double, double, void*),
        void *pp, double *error
) {
...
//==============================================================================================//
//            Routines that perform the full hadronic integration for regular terms.            //
//----------------------------------------------------------------------------------------------//
...
double integration::IntegrateDistrHadronic(
        int method,
