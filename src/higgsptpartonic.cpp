 ...
 *
 *    Description:  This file contains all the terms contribuing to the partonic Higgs 
 *                  fixed order results.
 *
 *        Version:  1.0
...
//----------------------------------------------------------------------------------------------//
void HiggsDpTpartonic::REG(double pt, double uh, double th, double sh, double q2) {
    ///////////////////////////////////////////////////////////
    ...
    REGgg = 1./std::pow(sh,2)/pt2/QQ2*(9.*(A0+Aep+A1234(pt,uh,th,sh,q2)+A1234(pt,th,uh,sh,q2) \
            +A3412(pt,uh,th,sh,q2)+A3412(pt,th,uh,sh,q2)+A1324(pt,uh,th,sh,q2) \
            +A1324(pt,th,uh,sh,q2)+A3241(pt,uh,th,sh,q2)+A3241(pt,th,uh,sh,q2)) \
            +4./3.*NF*(B1pmB1pp(pt,uh,th,sh,q2)+B1pmB1pp(pt,th,uh,sh,q2)) \
...
//                                      Coefficients                                            //
//----------------------------------------------------------------------------------------------//
void HiggsDpTpartonic::coeff(double pt, double uh, double th, double sh, double q2) {
    double tiny = 1e-4;
    ...
    big4 = 4.*((-std::pow(sh,3)*th-sh*std::pow(th,3)+std::pow(QQ2,3)*th+QQ2*std::pow(th,3)) \
        /sh/uh/th+xa*xb*(-std::pow(sh,3)*th/xa-sh*std::pow(th/xa,3)-std::pow(QQ2,3)*uh/xb-QQ2* \
        std::pow(uh/xb,3))/(sh*uh*th));

    // Term that gets multiplied by the seond NC*CF in Eq. (3.20) (inside square bracket)
    if (QQ2 > tmp) {
        big5 = 4.*((-std::pow(sh,3)*th/xa-sh*std::pow(th/xa,3)-std::pow(QQ2,3)*uh/xb-QQ2 \
            *std::pow(uh/xb,3))*(2.*QQ2+std::pow(pt,2))/std::pow(sh,2)/(QQ2+std::pow(pt,2)) \
            -2.*QQ2*std::pow(q2,2)*(std::pow(q2-th,2)+std::pow(th,2))/sh/uh/(q2-uh)) \
            *log(std::pow(pt,2)/(std::pow(pt,2)+QQ2))/(QQ2*std::pow(pt,2));
    } else {
...
//                                      A Functions                                             //
//----------------------------------------------------------------------------------------------//
double HiggsDpTpartonic::A1234(double pt, double u, double t, double s, double q2) {
    double pt2 = std::pow(pt,2);
...
double HiggsDpTpartonic::A3412(double pt, double u, double t, double s, double q2) {
    double pt2 = std::pow(pt,2);
    double q4 = std::pow(q2,2);
...
double HiggsDpTpartonic::A1324(double pt, double u, double t, double s, double q2) {
    double pt2 = std::pow(pt,2);
    double q4 = std::pow(q2,2);
    double q8 = std::pow(q2,4);
    double QQ2 = u+t+s-q2;
...
double HiggsDpTpartonic::A3241(double pt, double u, double t, double s, double q2) {
    double pt2 = std::pow(pt,2);
    double q4 = std::pow(q2,2);
    double QQ2 = u+t+s-q2;
    double x1 = t/(t-QQ2);
...
//                                      B Functions                                             //
//----------------------------------------------------------------------------------------------//
double HiggsDpTpartonic::B1pmB1pp(double pt, double u, double t, double s, double q2) {
    double pt2 = std::pow(pt,2);
    double QQ2 = u+t+s-q2;
    double x1 = t/(t-QQ2);
    double xQt2 = QQ2+pt2;
...
double HiggsDpTpartonic::B2pmB2pp(double pt, double u, double t, double s, double q2) {
    double pt2 = std::pow(pt,2);
    double q4 = std::pow(q2,2);
    double q6 = std::pow(q2,3);
    double QQ2 = u+t+s-q2;
    ...
    double B2pm = 1./3.*std::pow(t/x1,4)*pt2/xQt2*(std::pow(pt2,3)-std::pow(QQ2,3) \
                -std::pow(xQt2,3))/std::pow(xQt2,3)-s2*pt2*QQ2/3.;
...
//----------------------------------------------------------------------------------------------//
double HiggsDpTpartonic::Rgq(double pt, double u, double t, double s, double q2) {
    //////////////////////////////////////////////////////////////
...
double HiggsDpTpartonic::Rqqb(double pt, double u, double t, double s, double q2) {
    //////////////////////////////////////////////////////////////
    ...
    // Eqs. (A.28)-(A.34) G & S
    double D1pm = -s2*pt2*QQ2*(1.+log(pt2/xQt2))-s3*std::pow(pt,2)*x1*(1.-x1)/t \
                -s3*pt2*std::pow(1-x1,2);
    double D2pm = -s2*pt2*QQ2/3.-s*pt2*t2/6./std::pow(x1,2)*(11.-12.*QQ2/xQt2 \
                +3.*std::pow(QQ2/xQt2,2))+11.*s*pt2*t2/6.;
    double D1pp = s2*pt2*u2*(1-x2)*(L2b-L1b)/A+s2*pt2*std::pow(q2-u,2)*(1-x1)/A*(L2a-L1a) \
                +s*pt2*q2*QQ2*(s*pt2+u*t)/t2*L1a-2.*s2*pt2*q4*QQ2/A/B*L3+s*pt2*q2*QQ2 \
                *(2.*s*pt2-u*t)/t2;

    double D2pp = s*pt2*u2*(q2-t)*(1.-x2)/(2.*A)*(L1b-L2b)+s*pt2*std::pow(q2-u,3) \
    ...
    double E2 = s*pt2*QQ2*(QQ2-pt2)/std::pow(xQt2,2)*(std::pow(QQ2-t,2)+(std::pow(QQ2-u,2))) \
                +2.*s2*pt2*QQ2;
    double E3 = -2.*s*pt2*(std::pow(u+t-2.*QQ2,2)-2.*s*pt2)*log(pt2/xQt2)-s*pt2*QQ2*(2.*xQt2+QQ2) \
        /std::pow(xQt2,2)*(std::pow(QQ2-t,2)+(std::pow(QQ2-u,2)))-6.*s2*pt2*QQ2;

    double result  = (128./27.*(D1pm+D1pp)+32./3.*(D2pm+D2pp) \
                    +0.5*(16./9.*NF*E1+16./9.*E2+16./27.*E3))/s2/pt2/QQ2;
...
//**********************************************************************************************//
// The below compute the delta and regular terms that enter into the partonic cross section     //
//----------------------------------------------------------------------------------------------//
...
double HiggsDpTpartonic::deltapartonic(double pt, double nn, double zz) {
    ...
    // This function calculates the terms of the cross section proportional to delta(Q²)
    double QQ2 = 0;
    ...
    if (ORD >= 0) {
    switch (CHANNEL) {
        case(0): {result += gg0(sh,th,uh,MH2);}  // gg-channel
        break;
        case(1): {result += gq0(sh,th,uh);}     // gq-channel
        break;
        case(2): {result += qg0(sh,th,uh);}     // qg-channel
        break;
    ...
    if (ORD >= 1) {
    switch (CHANNEL) {
        // We multiply by aass/(2*M_PI) because it is one order higher than the LO part of the xsec
        // that is also calculated in this function. 
        case(0):    // gg-channel
        {
            double uu = 0.5*std::pow(log(uh/th),2)+std::pow(M_PI,2)/3. \
                -log(sh/MH2)*log(-th/MH2)-log(sh/MH2)*log(-uh/MH2) \
                -log(-uh/MH2)*log(-th/MH2)\
                +std::pow(log(MH2/sh),2)+std::pow(log(MH2/(MH2-th)),2) \
                +std::pow(log(MH2/(MH2-uh)),2)+2.*Li2(1.-MH2/sh) \
                +2.*Li2(MH2/(MH2-th))+2.*Li2(MH2/(MH2-uh));
            double de = 1.5*beta0*(log(-MUR2/th)+log(-MUR2/uh))+67./6.-5./9.*NF;
            result += aass/(2.*M_PI)*((11.+de+3.*uu)*gg0(sh,th,uh,MH2)
        ...
        case(1):    // gq-channel
        {
            double V1 = 0.5*(std::pow(log(uh/th),2)+std::pow(log(-sh/uh),2) \
                -std::pow(log(-sh/th),2))+log(sh/MH2)*log(-th/MH2) \
                -log(sh/MH2)*log(-uh/MH2)-log(-th/MH2)*log(-uh/MH2)+2. \
                *Li2(MH2/(MH2-uh))+std::pow(log(MH2/(MH2-uh)),2) \
                +std::pow(M_PI,2);
            double V2 = std::pow(log(MH2/sh),2)+std::pow(log(MH2/(MH2-th)),2) \
                -2.*log(sh/MH2)*log(-th/MH2)+2.*Li2(1.-MH2/sh)+2. \
                *Li2(MH2/(MH2-th))-3.5-2.*std::pow(M_PI,2)/3.;
            double V3 = beta0*(2*log(-MUR2/uh)+log(-MUR2/th))+67./3.-10.*NF/9.;

            result += aass/(2.*M_PI)*((11.+3*V1+4.*V2/3.+V3)*gq0(sh,th,uh) \
                    +20./9.*(std::pow(sh,2)+std::pow(th,2) \
                    +std::pow(uh,2)-uh*MH2)/(-uh));
        }
        break;
        case(2):    // qg-channel
        {
            double V1c = 0.5*(std::pow(log(th/uh),2)+std::pow(log(-sh/th),2) \
                -std::pow(log(-sh/uh),2))+log(sh/MH2)*log(-uh/MH2) \
                -log(sh/MH2)*log(-th/MH2)-log(-uh/MH2)*log(-th/MH2)+2. \
                *Li2(MH2/(MH2-th))+std::pow(log(MH2/(MH2-th)),2) \
                +std::pow(M_PI,2);
            double V2c = std::pow(log(MH2/sh),2)+std::pow(log(MH2/(MH2-uh)),2) \
                -2*log(sh/MH2)*log(-uh/MH2)+2.*Li2(1.-MH2/sh)+2. \
                *Li2(MH2/(MH2-uh))-3.5-2.*std::pow(M_PI,2)/3.;
            double V3c = beta0*(2*log(-MUR2/th)+log(-MUR2/uh))+67./3.-10.*NF/9.;
            result += aass/(2.*M_PI)*((11.+3.*V1c+4.*V2c/3.+V3c)*qg0(sh,th,uh) \
        ...
        {
            double W1 = log(-uh/MH2)*log(-th/MH2)-log(sh/MH2)*log(-uh/MH2)-log(sh/MH2) \
                *log(-th/MH2)+2.*Li2(1.-MH2/sh)+std::pow(log(MH2/sh),2)-0.5 \
                *std::pow(log(uh/th),2)-5.*std::pow(M_PI,2)/3.;
            double W2 = 1.5*log(std::pow(sh,2)/th/uh)+std::pow(log(uh/th),2) \
                -2.*log(-uh/MH2)*log(-th/MH2)+std::pow(log(MH2/(MH2-uh)),2) \
                +std::pow(log(MH2/(MH2-th)),2)+2.*Li2(MH2/(MH2-uh))+2. \
                *Li2(MH2/(MH2-th))-7.+2.*std::pow(M_PI,2);
            double W3 = beta0/2*(4*log(MUR2/sh)+log(-MUR2/uh)+log(-MUR2/th)) \
                +(67./2.-5.*NF/3.);
            result += aass/(2.*M_PI)*((11.+3.*W1+4.*W2/3.+W3)*qqb0(sh,th,uh)+160./27. \
                    *(std::pow(th,2)+std::pow(uh,2)+std::pow(sh,2)-sh*MH2)/sh);
        }
        break;
    }
    }
