 ...
 *
 *    Description:  This file computes the product of two PDFs, for convenience,
 * we call the product luminosity.
 *
 *        Version:  1.0
...
Luminosity::Luminosity(std::string pdfname, double MUF, int NF) {
  _nf = NF;
  _muf = MUF;
...
// Extract alphas //
double Luminosity::get_alphas(double mur) { return _pdf->alphasQ(mur); }
...
double Luminosity::Lumgq(double x1, double x2) {
  ...
  for (int i = 0; i < _nf; i++) {
    lgq += (fxQ(x2, (i + 1)) + fxQ(x2, -(i + 1))) * fxQ(x1, 0);
  }
  return lgq;
}

double Luminosity::Lumqg(double x1, double x2) { return Lumgq(x2, x1); }
