 ...
 *
 *    Description:  This file contains all the terms contribuing to the Higgs
 * fixed order results.
 *
 *        Version:  1.0
...
HiggsDpT::HiggsDpT(int order, int channel, std::string pdfname, void *params) {
  ...
  Lumi = new Luminosity(pdfname, param.mur, param.nf);
}
...
void HiggsDpT::REG(double pt, double uh, double th, double sh) {
  ...
  REGgg =
      ...
      (9. * (A0 + Aep + A1234(pt, uh, th, sh) + A1234(pt, th, uh, sh) +
             A3412(pt, uh, th, sh) + A3412(pt, th, uh, sh) +
             A1324(pt, uh, th, sh) + A1324(pt, th, uh, sh) +
             A3241(pt, uh, th, sh) + A3241(pt, th, uh, sh)) +
       4. / 3. * NF * (B1pmB1pp(pt, uh, th, sh) + B1pmB1pp(pt, th, uh, sh)) +
       3. * NF * (B2pmB2pp(pt, uh, th, sh) + B2pmB2pp(pt, th, uh, sh)));
...
double HiggsDpT::Rqqb(double pt, double u, double t, double s) {
  ...
  // Eqs. (A.28)-(A.34) G & S
  D1pm = -s2 * pt2 * QQ2 * (1. + log(pt2 / xQt2)) -
  ...
  E2 = s * pt2 * QQ2 * (QQ2 - pt2) / std::pow(xQt2, 2) *
           (std::pow(QQ2 - t, 2) + (std::pow(QQ2 - u, 2))) +
       2. * s2 * pt2 * QQ2;
  E3 = -2. * s * pt2 * (std::pow(u + t - 2. * QQ2, 2) - 2. * s * pt2) *
           log(pt2 / xQt2) -
       s * pt2 * QQ2 * (2. * xQt2 + QQ2) / std::pow(xQt2, 2) *
           (std::pow(QQ2 - t, 2) + (std::pow(QQ2 - u, 2))) -
       6. * s2 * pt2 * QQ2;

  result = (128. / 27. * (D1pm + D1pp) + 32. / 3. * (D2pm + D2pp) +
...
//**********************************************************************************************//
// The below compute the delta and regular terms that enter into the hadronic
// cross section     // i.e. the partonic part convoluted with the PDFs and
// integrated over the momentunm fractions. // The integration w.r.t. the
// momentum fractions follow Section B of G & S.                     //
//----------------------------------------------------------------------------------------------//

//==============================================================================================//
//                                        Delta Terms //
//----------------------------------------------------------------------------------------------//
double HiggsDpT::delta(double pt, double tm, double t, double y1, double y2,
                       double yc) {
  ///////////////////////////////////////////////////////////////
  ...
  double x1 =
      (SROOT * tm * exp(yh) * x2 - q2) / (x2 * shad - SROOT * tm * exp(-yh));
  ...
  if (ORD >= 1) {
    ...
    double uu = 0.5 * std::pow(log(uh / th), 2) + std::pow(M_PI, 2) / 3. -
                ...
                log(-uh / q2) * log(-th / q2) + std::pow(log(q2 / sh), 2) +
                std::pow(log(q2 / (q2 - th)), 2) +
    ...
    double V1 = 0.5 * (std::pow(log(uh / th), 2) + std::pow(log(-sh / uh), 2) -
                       std::pow(log(-sh / th), 2)) +
                log(sh / q2) * log(-th / q2) - log(sh / q2) * log(-uh / q2) -
                log(-th / q2) * log(-uh / q2) + 2. * Li2(q2 / (q2 - uh)) +
                std::pow(log(q2 / (q2 - uh)), 2) + std::pow(M_PI, 2);
    double V2 = std::pow(log(q2 / sh), 2) + std::pow(log(q2 / (q2 - th)), 2) -
                2. * log(sh / q2) * log(-th / q2) + 2. * Li2(1. - q2 / sh) +
                2. * Li2(q2 / (q2 - th)) - 3.5 - 2. * std::pow(M_PI, 2) / 3.;
    double V3 = beta0 * (2 * log(-MUR2 / uh) + log(-MUR2 / th)) + 67. / 3. -
                10. * NF / 9.;

    double ddgq =
        ((11. + 3 * V1 + 4. * V2 / 3. + V3) * gq0(x1, x2, tm, yh) +
         20. / 9. *
             (std::pow(sh, 2) + std::pow(th, 2) + std::pow(uh, 2) - uh * q2) /
             (-uh)) *
        Lumi->Lumgq(x1, x2);
    ...
    double V1c = 0.5 * (std::pow(log(th / uh), 2) + std::pow(log(-sh / th), 2) -
                        std::pow(log(-sh / uh), 2)) +
                 log(sh / q2) * log(-uh / q2) - log(sh / q2) * log(-th / q2) -
                 log(-uh / q2) * log(-th / q2) + 2. * Li2(q2 / (q2 - th)) +
                 std::pow(log(q2 / (q2 - th)), 2) + std::pow(M_PI, 2);
    double V2c = std::pow(log(q2 / sh), 2) + std::pow(log(q2 / (q2 - uh)), 2) -
                 2 * log(sh / q2) * log(-uh / q2) + 2. * Li2(1. - q2 / sh) +
                 2. * Li2(q2 / (q2 - uh)) - 3.5 - 2. * std::pow(M_PI, 2) / 3.;
    double V3c = beta0 * (2 * log(-MUR2 / th) + log(-MUR2 / uh)) + 67. / 3. -
                 10. * NF / 9.;

    double ddqg =
        ((11. + 3. * V1c + 4. * V2c / 3. + V3c) * qg0(x1, x2, tm, yh) +
         20. / 9. *
             (std::pow(sh, 2) + std::pow(th, 2) + std::pow(uh, 2) - th * q2) /
             (-th)) *
        Lumi->Lumqg(x1, x2);

    // qqbar-channel //
    // W functions Eqs. (3.26)-(3.28)
    double W1 = log(-uh / q2) * log(-th / q2) - log(sh / q2) * log(-uh / q2) -
                log(sh / q2) * log(-th / q2) + 2. * Li2(1. - q2 / sh) +
                std::pow(log(q2 / sh), 2) - 0.5 * std::pow(log(uh / th), 2) -
                5. * std::pow(M_PI, 2) / 3.;
    double W2 = 1.5 * log(std::pow(sh, 2) / th / uh) +
                std::pow(log(uh / th), 2) - 2. * log(-uh / q2) * log(-th / q2) +
                std::pow(log(q2 / (q2 - uh)), 2) +
                std::pow(log(q2 / (q2 - th)), 2) + 2. * Li2(q2 / (q2 - uh)) +
                2. * Li2(q2 / (q2 - th)) - 7. + 2. * std::pow(M_PI, 2);
    double W3 =
        beta0 / 2 * (4 * log(MUR2 / sh) + log(-MUR2 / uh) + log(-MUR2 / th)) +
        (67. / 2. - 5. * NF / 3.);
    // NOTE: are we missing the lumi here?
    double ddqqb =
        ((11. + 3. * W1 + 4. * W2 / 3. + W3) * qqb0(x1, x2, tm, yh) +
         160. / 27. *
             (std::pow(th, 2) + std::pow(uh, 2) + std::pow(sh, 2) - sh * q2) /
             sh);
...
//                                    Non-Delta Terms //
//----------------------------------------------------------------------------------------------//
double HiggsDpT::distr(double pt, double tm, double y1, double y2, double zz1,
                       double zz2, double zz3) {
  ////////////////////////////////////////////////////////////////
  ...
  double dcut =
      x10 * std::pow(pt / tm, 2) / (1. - x10 * (1. - std::pow(pt / tm, 2)));
  ...
  coeff(pt, uh, th, sh);
  double a1 =
      (1. / (-th) * pgg(z2) * gg0(z2 * x1, x2, tm, yh) + (-z2 / th) * big1) *
      Lumi->Lumgg(x1, x2) / sh;
  double b1 =
      (1. / th * pgg(z2) * log(-MUF2 * z2 / th) * gg0(z2 * x1, x2, tm, yh) +
       z2 / th * big1 * log((QQ2 + pt * pt) * z2 / (-th)) + z2 / th * big2) *
      Lumi->Lumgg(x1, x2) / sh;
  double c1 =
      (1 / (-th) *
           (-2 * NF * Pqg(z2) * log(MUF2 / QQ2) + 2 * NF * z2 * (1 - z2)) *
           qg0(z2 * x1, x2, tm, yh) +
       0.5 * big3) *
      Lumi->Lumgg(x1, x2) / sh;
  ...
  coeff(pt, th, uh, sh);
  a1 +=
      -1. / th * pqq(z2) * qg0(z2 * x1, x2, tm, yh) * Lumi->Lumqg(x1, x2) / sh;
  b1 += (1. / th * pqq(z2) * log(-MUF2 * z2 / th) * qg0(z2 * x1, x2, tm, yh) +
         z2 / th * 8. / 3. * (std::pow(uh, 2) + std::pow(sh, 2)) / (-th)) *
        Lumi->Lumqg(x1, x2) / sh;
  c1 += (-1 / th *
             (4. / 3. * (1 - z2) * qg0(z2 * x1, x2, tm, yh) +
              (-Pgq(z2) * log(MUF2 / QQ2) + 4 / 3 * z2) *
                  gg0(z2 * x1, x2, tm, yh)) +
         0.5 * big5) *
        Lumi->Lumqg(x1, x2) / sh;
  ...
  b1 +=
      (1. / th * pqq(z2) * log(-MUF2 * z2 / th) * qqb0(z2 * x1, x2, tm, yh) -
       z2 / th * log((QQ2 + std::pow(pt, 2)) * z2 / (-th)) * 16. / 27. *
           (std::pow(th, 2) + std::pow(uh, 2) + std::pow(QQ2 - th, 2) +
            std::pow(QQ2 - uh, 2)) /
           sh +
       z2 / th * 16. / 9. * beta0 * (std::pow(uh, 2) + std::pow(th, 2)) / sh) *
      Lumi->Lumqqb(x1, x2) / sh;
  c1 +=
      (-1 / th *
           (4. / 3 * (1 - z2) * qqb0(z2 * x1, x2, tm, yh) +
            (-Pgq(z2) * log(MUF2 / QQ2) + 4. / 3 * z2) *
                gq0(z2 * x1, x2, tm, yh)) +
  ...
  c1 +=
      (-1. / th * (-Pgq(z2) * log(MUF2 / QQ2) + 4. / 3 * z2) *
           gq0(z2 * x1, x2, tm, yh) +
       16. / 9 * (std::pow(sh - QQ2, 2) + std::pow(uh + th - 2 * QQ2, 2)) / sh *
           log(std::pow(pt, 2) / (std::pow(pt, 2) + QQ2)) / std::pow(pt, 2)) *
  ...
  double nosingular =
      0.5 *
      (0. + REGgg * Lumi->Lumgg(x1, x2) + REGgq * Lumi->Lumgq(x1, x2) +
       REGqg * Lumi->Lumqg(x1, x2) + REGqqb * Lumi->Lumqqb(x1, x2) +
       REGqqpb * (Lumi->LumqQ(x1, x2) + Lumi->LumqQb(x1, x2)) +
  ...
  double a10 = (1 / (-th) * pgg(1.) * gg0(x1, x2, tm, yh) + (-1. / th) * big1) *
               Lumi->Lumgg(x1, x2) / sh;
  double b10 = (6. / th * log(-MUF2 / th) * gg0(x1, x2, tm, yh) +
                big1 / th * log((xQt2) / (-th)) + big2 / th) *
               Lumi->Lumgg(x1, x2) / sh;
  ...
  a10 += (1 / (-th) * pgg(1.) * gq0(x1, x2, tm, yh) + (-1. / th) * big4) *
         Lumi->Lumgq(x1, x2) / sh;
  b10 += (1. / th * pgg(1.) * log(-MUF2 / th) * gq0(x1, x2, tm, yh) +
          1. / th * big4 * log((xQt2) / (-th))) *
         Lumi->Lumgq(x1, x2) / sh;
  ...
  b10 += (1. / th * pqq(1.) * log(-MUF2 / th) * qg0(x1, x2, tm, yh) +
          1. / th * 8. / 3. * (std::pow(uh, 2) + std::pow(sh, 2)) / (-th)) *
         Lumi->Lumqg(x1, x2) / sh;
  d10 += 1. / th * 2. * log(MUF2 / (-th)) * qg0(x1, x2, tm, yh) *
         Lumi->Lumqg(x1, x2) / sh;

  // qqb-channel
  a10 += 1. / (-th) *
         (pqq(1) * qqb0(x1, x2, tm, yh) -
          16. / 27. *
              (std::pow(th, 2) + std::pow(uh, 2) + std::pow(QQ2 - th, 2) +
               std::pow(QQ2 - uh, 2)) /
              sh) *
         Lumi->Lumqqb(x1, x2) / sh;
  b10 +=
      (1. / th * pqq(1.) * log(-MUF2 / th) * qqb0(x1, x2, tm, yh) -
       1. / th * log((xQt2) / (-th)) * 16. / 27. *
           (std::pow(th, 2) + std::pow(uh, 2) + std::pow(QQ2 - th, 2) +
            std::pow(QQ2 - uh, 2)) /
           sh +
       1. / th * 16. / 9. * beta0 * (std::pow(uh, 2) + std::pow(th, 2)) / sh) *
      Lumi->Lumqqb(x1, x2) / sh;
  d10 += 1. / th * 2. * log(MUF2 / (-th)) * qqb0(x1, x2, tm, yh) *
         Lumi->Lumqqb(x1, x2) / sh;
...
//                                    Non-Delta Terms //
//----------------------------------------------------------------------------------------------//
double HiggsDpT::distrcross(double pt, double tm, double y1, double y2,
  ...
  double x20 = tm / SROOT * exp(-yh);
  double d =
      x20 * std::pow(pt / tm, 2) / (1. - x20 * (1. - std::pow(pt / tm, 2)));
  ...
  coeff(pt, uh, th, sh);
  double a1 =
      (1. / (-uh) * pgg(z2) * gg0(x1, x2 * z2, tm, yh) + (-z2 / uh) * big1) *
      Lumi->Lumgg(x1, x2) / sh;
  double b1 =
      (1. / uh * pgg(z2) * log(-MUF2 * z2 / uh) * gg0(x1, x2 * z2, tm, yh) +
  ...
  b1 += (1. / uh * pqq(z2) * log(-MUF2 * z2 / uh) * qqb0(x1, z2 * x2, tm, yh) -
         z2 / uh * log((QQ2 + std::pow(pt, 2)) * z2 / (-uh)) * 16. / 27 *
             (std::pow(th, 2) + std::pow(uh, 2) + std::pow(QQ2 - th, 2) +
              std::pow(QQ2 - uh, 2)) /
             sh +
         z2 / uh * 16. / 9 * beta0 * (std::pow(uh, 2) + std::pow(th, 2)) / sh) *
        Lumi->Lumqqb(x1, x2) / sh;
  c1 +=
      (-1. / uh *
           (4. / 3 * (1 - z2) * qqb0(x1, z2 * x2, tm, yh) +
            (-Pgq(z2) * log(MUF2 / QQ2) + 4. / 3 * z2) *
                qg0(x1, z2 * x2, tm, yh)) +
       16. / 9 * (std::pow(sh - QQ2, 2) + std::pow(uh + th - 2 * QQ2, 2)) / sh *
           log(std::pow(pt, 2) / (std::pow(pt, 2) + QQ2)) / std::pow(pt, 2)) *
  ...
  c1 +=
      (-1. / uh * (-Pgq(z2) * log(MUF2 / QQ2) + 4. / 3 * z2) *
           qg0(x1, z2 * x2, tm, yh) +
       16. / 9 * (std::pow(sh - QQ2, 2) + std::pow(uh + th - 2 * QQ2, 2)) / sh *
           log(std::pow(pt, 2) / (std::pow(pt, 2) + QQ2)) / std::pow(pt, 2)) *
      (Lumi->Lumqq(x1, x2) + Lumi->LumqQb(x1, x2) + Lumi->LumqQ(x1, x2)) / sh;
  ...
  double nosingular =
      0.5 *
      (REGgg * Lumi->Lumgg(x1, x2) + REGgq * Lumi->Lumgq(x1, x2) +
       REGqg * Lumi->Lumqg(x1, x2) + REGqq * Lumi->Lumqq(x1, x2) +
  ...
  double a10 = (1. / (-uh) * pgg(1.) * gg0(x1, x2, tm, yh) + (-1 / uh) * big1) *
               Lumi->Lumgg(x1, x2) / sh;
  double b10 =
      (1. / uh * pgg(1.) * log(-MUF2 / uh) * gg0(x1, x2, tm, yh) +
  ...
  a10 += 1. / (-uh) *
         ...
         Lumi->Lumqqb(x1, x2) / sh;
  b10 +=
      (1. / uh * pqq(1.) * log(-MUF2 / uh) * qqb0(x1, x2, tm, yh) -
       1. / uh * log((QQ2 + std::pow(pt, 2)) / (-uh)) * 16. / 27 *
           (std::pow(th, 2) + std::pow(uh, 2) + std::pow(QQ2 - th, 2) +
            std::pow(QQ2 - uh, 2)) /
           sh +
       1. / uh * 16. / 9 * beta0 * (std::pow(uh, 2) + std::pow(th, 2)) / sh) *
      Lumi->Lumqqb(x1, x2) / sh;
  ...
  double m10 =
      -0.5 * std::pow(log(1. - z2min), 2) * a10 - log(1. - z2min) * b10;
  m10 *= pre10 * (1. - d - x10);
