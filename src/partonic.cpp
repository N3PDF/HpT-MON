 ...
 *
 *    Description:  This file provide the partonic results in N space for a given channel.
 *
 *        Version:  1.0
...
double CrossHiggs::ExtractDeltaPartonic(
        double pt,
        double nn,
        double zz
) {
...
double PartonicDeltaIntegrand(double zz, void *p) {
    ...
    return par.croshiggs->ExtractDeltaPartonic(
            par.pt,
            par.nn,
            zz);
}
...
//----------------------------------------------------------------------------------------------//
double CrossHiggs::ExtractDistrPartonic(
        double pt,
        double nn,
        double zz1,
        double zz2
...
double PartonicDistrIntegrand(double zz1, double zz2, void *p) {
    PartStruct par = *reinterpret_cast<PartStruct*>(p);
    return par.croshiggs->ExtractDistrPartonic(
            par.pt,
            par.nn,
            zz1,
            zz2);
...
//----------------------------------------------------------------------------------------------//
std::vector<double> CrossHiggs::partonichiggsdpt(
        double pt,
        double nn
) {
    //////////////////////////////////////////////////
    ...
    // LO
    deltres = integration::IntegrateDeltaPartonic(
            method,
            PartonicDeltaIntegrand,
            &finalparams,
            &delterr);
